...
def modify_pdfs_from_excel(excel_path, text):
    df = pd.read_excel(excel_path, usecols=[0, 1, 3])  # Read the specified columns from the Excel file
    for index, row in df.iterrows():  # Loop through each row in the DataFrame
        filename = '%0*d' % (6, int(row[0])) + '.pdf'  # Format the filename based on the 'count' column
        ann = f"{int(row[1])}       {text}"  # Create the annotation text 
        output_pdf = '_' + '%0*d' % (2, index+1) + '_' + f"{row[2]:,.1f}_" + filename 
...
if __name__ == "__main__":  # If the script is executed directly
    excel_path = r'.\021-10_abcant.xlsx'  # Define the path to the Excel file
    text = "J.24.021-10"
